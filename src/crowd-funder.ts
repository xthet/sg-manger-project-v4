...
const cdf = "0xb9e150eBeB6095781F13c89f27749D894Ef03071"

export function handleCampaignAdded(event: CampaignAddedEvent): void {
	let campaignAdded = CampaignAdded.load(
	...
	if (!campaignAdded) {
		campaignAdded = new CampaignAdded(
			event.params._campaignAddress.toHexString()
		)
...
export function handleCampaignFunded(event: CampaignFundedEvent): void {
	...
	crowdFunder.donationCount = crowdFunder.donationCount!.plus(
		BigInt.fromString("1")
	)
	...
	if (!campaignAdded!.funders.includes(event.params._funder)) {
		...
		campaignAdded!.funderCount = campaignAdded!.funderCount.plus(
			BigInt.fromString("1")
		)
	}
...
export function handleCampaignShrunk(event: CampaignShrunkEvent): void {
	...
	if (
		campaignAdded!.funderCount.gt(BigInt.fromString("0")) &&
		campaignAdded!.funders.includes(event.params._withdrawer)
	) {
		// if funder in array and funders > 0
		campaignAdded!.funderCount = campaignAdded!.funderCount.minus(
			BigInt.fromString("1")
		)
	}
	...
	if (i_withdrawer.backedCount!.gt(BigInt.fromString("0"))) {
		i_withdrawer.backedCount = i_withdrawer.backedCount!.minus(
			BigInt.fromString("1")
		)
	}
	...
	crowdFunder.donationCount = crowdFunder.donationCount!.minus(
		BigInt.fromString("1")
	)
...
export function handleCampaignPublished(event: CampaignPublishedEvent): void {
	...
	if (userAdded.publishedCount!.equals(BigInt.fromString("0"))) {
		// on first publish
		crowdFunder.creatorCount = crowdFunder.creatorCount!.plus(
			BigInt.fromString("1")
		)
	}

	userAdded.publishedCount = userAdded.publishedCount!.plus(
		BigInt.fromString("1")
	)
	// only published

	crowdFunder.campaignCount = crowdFunder.campaignCount!.plus(
		BigInt.fromString("1")
	)
